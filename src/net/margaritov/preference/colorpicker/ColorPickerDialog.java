...
public class ColorPickerDialog extends Dialog implements
    ...
    public ColorPickerDialog(Context context, int theme, int initialColor,
            int androidColor, int octosColor) {
        super(context, theme);
        mInitialColor = initialColor;
        mAndroidColor = androidColor;
        mOctosColor = octosColor;
        if (mAndroidColor != 0x00000000 && mOctosColor != 0x00000000) {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.palette_octos) {
            mPalette = PALETTE_OCTOS;
            mColorTransitionAnimator.start();
            return true;
            ...
            return true;
        } else if (item.getItemId() == R.id.reset_octos) {
            mColorPicker.setColor(mOctosColor, true);
            return true;
        }
        return false;
    }
